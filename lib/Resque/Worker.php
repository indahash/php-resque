...
class Resque_Worker
{
    ...
    /**
     * @var int Number of jobs fun by worker
     */
    public $counter = 0;
    ...
    public function work($interval = Resque::DEFAULT_INTERVAL, $blocking = false)
    {
        ...
        while(true) {
            ...
            $COUNTER = getenv('JOB_LIMIT');
            if (!empty($COUNTER)) {
                if ($this->counter >= (int)$COUNTER) {
                    $this->logger->log(Psr\Log\LogLevel::NOTICE, 'Shutting down worker. Job limit exceeded {limit}', array('limit' => $COUNTER));
                    $this->shutdown();
                }
            }
    ...
    public function registerWorker()
    {
        ...
        Resque::redis()->set('worker:' . (string)$this . ':started', (new DateTime())->format('D M d H:i:s e Y'));
    }
    ...
    public function workingOn(Resque_Job $job)
    {
        ...
        $data = json_encode(array(
            'queue' => $job->queue,
            'run_at' => (new DateTime())->format('D M d H:i:s e Y'),
            'payload' => $job->payload
        ));
